...
@WebServlet("/bookInternationalFlight")
public class InternationalBookingServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM international_flights");
			rs = ps.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
		}
        ...
        try {
            ...
            if (success) {
            	...
            	success = bookingDAO.bookFlight(flightId, passengerName, seatClass, numPassengers, isDomestic, paid, users, ArrivalTime, DepartureTime);
    	   ...
    	   request.setAttribute("booking_failed", "booking_failed");
    	   request.getRequestDispatcher("jsp/BookingConfirmation_Inter.jsp").forward(request, response);
       }
       }
    }
